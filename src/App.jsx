...
import { lessons, questions, sources } from './data'
...
function HomePage({go,done}){
  ...
  return <section className="page home-page">
    ...
    <div className="stats"><article><Languages/><div><b>18</b><span>Pelajaran Irodori</span></div></article><article><Brain/><div><b>{questions.length}</b><span>Soal latihan</span></div></article><article><Trophy/><div><b>{Math.round(done.length/18*100)}%</b><span>Progres materi</span></div></article></div>
...
function Quiz(){
  const pool=useMemo(()=>shuffle(questions).slice(0,8),[]); const [idx,setIdx]=useState(0); const [picked,setPicked]=useState(null); const [score,setScore]=useState(0); const [finished,setFinished]=useState(false); const q=pool[idx]
  const choose=(i)=>{if(picked!==null)return;setPicked(i);if(i===q.answer)setScore(score+1)}
  const next=()=>{if(idx===pool.length-1)setFinished(true);else{setIdx(idx+1);setPicked(null)}}
  if(finished)return <Result score={score} total={pool.length} reset={()=>location.reload()} label="Hasil kuis"/>
  return <section className="page quiz-page"><div className="quiz-head"><div><span className="eyebrow">KUIS ADAPTIF • PEMBAHASAN LANGSUNG</span><h1>Latihan tanpa tekanan.</h1></div><span className="score-badge">{score} benar</span></div><div className="progress"><i style={{width:`${(idx+1)/pool.length*100}%`}}/></div><article className="question-card"><div className="question-meta"><span>{q.section}</span><b>{idx+1} / {pool.length}</b></div><h2>{q.q}</h2><div className="options">{q.options.map((o,i)=><button key={o} className={picked===null?'':i===q.answer?'correct':picked===i?'wrong':'muted'} onClick={()=>choose(i)}><span>{String.fromCharCode(65+i)}</span>{o}{picked!==null&&i===q.answer?<Check/>:null}</button>)}</div>{picked!==null&&<div className={picked===q.answer?'feedback good':'feedback bad'}><b>{picked===q.answer?'正解！ Tepat sekali.':'Belum tepat — mari pahami.'}</b><p>{q.why}</p></div>}<button className="primary next" disabled={picked===null} onClick={next}>{idx===pool.length-1?'Lihat hasil':'Soal berikutnya'} <ChevronRight/></button></article></section>
}

function Exam(){
  const exam=useMemo(()=>shuffle(questions).map(q=>({...q,options:q.options.map((x,i)=>({x,ok:i===q.answer})).sort(()=>Math.random()-.5)})),[])
  const [started,setStarted]=useState(false),[idx,setIdx]=useState(0),[answers,setAnswers]=useState({}),[time,setTime]=useState(15*60),[done,setDone]=useState(false)
  ...
  if(!started)return <section className="page exam-intro"><div className="exam-mark">試</div><span className="eyebrow">SIMULASI • LATIHAN NON-RESMI</span><h1>Siap mengukur kemampuanmu?</h1><p>Format disederhanakan dari empat bagian JFT-Basic. Kerjakan {exam.length} soal dalam 15 menit tanpa pembahasan selama ujian.</p><div className="exam-facts"><div><Clock3/><b>15 menit</b><span>Timer otomatis</span></div><div><CircleHelp/><b>{exam.length} soal</b><span>Empat keterampilan</span></div><div><Trophy/><b>70%</b><span>Target latihan</span></div></div><button className="primary" onClick={()=>setStarted(true)}>Mulai simulasi <ChevronRight/></button></section>
  const score=Object.entries(answers).filter(([i,v])=>exam[i].options[v]?.ok).length
  if(done)return <Result score={score} total={exam.length} reset={()=>location.reload()} label="Hasil simulasi"/>
  const q=exam[idx]; const mm=String(Math.floor(time/60)).padStart(2,'0'),ss=String(time%60).padStart(2,'0')
  return <section className="page exam-page"><div className="exam-bar"><div><span className="eyebrow">SIMULASI JFT-BASIC</span><b>{q.section}</b></div><span className={time<120?'timer urgent':'timer'}><Clock3/>{mm}:{ss}</span><button onClick={()=>setDone(true)}>Selesaikan</button></div><div className="exam-layout"><div className="number-grid">{exam.map((_,i)=><button key={i} className={`${i===idx?'now':''} ${answers[i]!==undefined?'answered':''}`} onClick={()=>setIdx(i)}>{i+1}</button>)}</div><article className="question-card"><div className="question-meta"><span>SOAL {idx+1}</span><b>{Object.keys(answers).length}/{exam.length} terjawab</b></div><h2>{q.q}</h2><div className="options">{q.options.map((o,i)=><button key={o.x} className={answers[idx]===i?'selected-option':''} onClick={()=>setAnswers({...answers,[idx]:i})}><span>{String.fromCharCode(65+i)}</span>{o.x}</button>)}</div><div className="exam-nav"><button disabled={idx===0} onClick={()=>setIdx(idx-1)}><ChevronLeft/> Sebelumnya</button><button className="primary" onClick={()=>idx===exam.length-1?setDone(true):setIdx(idx+1)}>{idx===exam.length-1?'Selesai':'Berikutnya'} <ChevronRight/></button></div></article></div></section>
}
